...
This is straightforward using the mipreadr workflow. We first read the files into R, then automatically the technical replicates for the samples have the blank values subtracted and are averaged based on the plate layouts provided. The `normalizeFluorescence` argument generates a new column for GFP/OD values. Afte this, the biological samples from different experiments are also collapsed. This is all done with the `addPlate` and `mergePlates()` functions.
...
In order to have have more flexibility for plotting, we will append some metadata - including a custom color palette - onto the analysis data frame:
